...
def generate_customers(num_customers=10000, start_date="2022-01-01"):
    ...
    channels = ['organic', 'paid_search', 'social_media', 'referral', 'email_campaign', 'direct']
...
def generate_orders(customers, orders_per_customer_range=(1, 50)):
    ...
    for _, customer in customers.iterrows():
        ...
        customer_age_days = (datetime.now() - datetime.strptime(customer['signup_date'], '%Y-%m-%d')).days
        ...
        for i in range(num_orders):
            ...
            # Product categories
            categories = ['software_license', 'consulting', 'support', 'training', 'additional_features']
            ...
            if i == 0:
                order_type = 'new'
            elif i >= num_orders - 2 and np.random.random() < 0.1:  # 10% chance of churn
                order_type = 'churn'
...
def generate_events(customers, events_per_customer_range=(10, 100)):
    ...
    event_types = [
        'login', 'feature_usage', 'purchase', 'support_ticket',
        'page_view', 'download', 'webinar_attended', 'demo_request'
    ]
    ...
    for _, customer in customers.iterrows():
        ...
        total_events = int(customer_age_days * events_per_day * np.random.uniform(0.5, 1.5))
...
def main():
    ...
    subscriptions_df.to_parquet(data_dir / "subscriptions.parquet", index=False)
    ...
    paying_customers = orders_df['customer_id'].nunique()

    print(f"\nRevenue Metrics:")
    print(f"Total Revenue: ${total_revenue:,.2f}")
    print(f"Average Order Value: ${avg_order_value:.2f}")
    print(f"Paying Customers: {paying_customers:,}")
    print(f"Conversion Rate: {paying_customers/len(customers_df)*100:.1f}%")
