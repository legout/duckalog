...
def ui(
    ...
    try:
        ...
        _fail(
            f"UI dependencies not installed. Install with: pip install duckalog[ui]\n"
            f"Missing: {exc.name}",
            2
        )
    ...
    dashboard_app = create_app(ctx)

    typer.echo(f"Starting dashboard at http://{host}:{port}")
    typer.echo(f"Config: {config_path}")
    typer.echo(f"Database: {config.duckdb.database}")
    typer.echo(f"Views: {len(config.views)}")
